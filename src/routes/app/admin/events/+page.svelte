<script lang="ts">
	...
	const saveImage = async (url: string) => {
		...
		await supabase.from('events').update({ base64: imageData.base64 }).eq('id', events[current].id);
...
<!-- <div class="flex flex-col space-y-8 p-16 text-2xl">
	...
	<p class="text-base">{events?.[current]?.summary}</p>

	<div class="flex flex-row">
		<input type="text" bind:value={query} class="w-1/2 border-2 border-black p-4 text-2xl" />
